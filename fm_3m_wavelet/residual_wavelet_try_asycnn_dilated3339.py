...
#==============================================================================
# Wavelet-based 2D Convolutional Feature Extractor (MODIFIED FOR DILATION)
#==============================================================================

class Conv2dResidualBlock(nn.Module):
    def __init__(self, in_channels: int, out_channels: int, stride: int = 1, activation: Callable = nn.GELU(), dilation_rate: int = 1):
        super().__init__()
        
        # --- NEW CHANGE: Use DILATED convolutions to increase receptive field ---
        kernel_size = (3, 9) # A moderately wide kernel
        
        # The dilation is applied only to the time axis (width)
        dilation = (1, dilation_rate)
        
        # Padding formula to maintain size: padding = dilation * (kernel_size - 1) / 2
        ...
        self.main_path = nn.Sequential(
            ...
            activation,
            # We can use a larger dilation in the second layer of the block
            nn.Conv2d(out_channels, out_channels, kernel_size=kernel_size, stride=1, padding=padding, dilation=dilation, bias=False),
            nn.BatchNorm2d(out_channels)
        )
...
class SignalConvEmbedding(nn.Module):
    def __init__(self, output_embedding_dim: int):
        ...
        self.initial_layer = nn.Sequential(
            nn.Conv2d(1, self.in_channels, kernel_size=(3, 3), stride=2, padding=(1, 1), bias=False),
            nn.BatchNorm2d(self.in_channels),
            nn.GELU(),
        )
        
        # --- NEW CHANGE: Pass increasing dilation rates to each layer ---
        self.layer1 = self._make_layer(64, 2, stride=1, dilation_rate=1)
        self.layer2 = self._make_layer(128, 2, stride=2, dilation_rate=2)
        self.layer3 = self._make_layer(256, 1, stride=2, dilation_rate=4)
        self.layer4 = self._make_layer(512, 1, stride=2, dilation_rate=8)
    ...
    def _make_layer(self, out_channels, num_blocks, stride, dilation_rate=1):
        strides = [stride] + [1] * (num_blocks - 1)
        layers = []
        for s in strides:
            # Pass the dilation rate to the residual block
            layers.append(Conv2dResidualBlock(self.in_channels, out_channels, stride=s, dilation_rate=dilation_rate))
