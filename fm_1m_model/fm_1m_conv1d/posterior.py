...
def sample_parameters(model, signal, device, x_0=None, num_sample=1000):
    ...
    signal = signal.float()
    if signal.shape[0] != 1:
        raise ValueError("Only one signal can be passed at a time")
    
    # Repeat the signal for parallel sampling
...
def analyze_posterior(model, signal, true_params, parameters_std, parameters_mean, 
